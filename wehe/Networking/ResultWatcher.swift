...
class ResultWatcher {
    ...
    func requestAnalysis(testID: Int) {
        ...
        print("DEBUG! Sending result request", analysisUrl, annalysisIP)
    ...
    private func getResults(testID: Int) {
        ...
        session.request(analysisUrl, parameters: parameters).responseJSON(queue: queue) { response in
            ...
            case .failure:
                ...
                sleep(UInt32(self.sleepConstant))
                self.getResults(testID: testID)
            }
        }
    }
    ...
    // Localization: changed from 'private func' to 'func' (Localization.swift uses this method now)
    func handleResult(json: JSON, appName: String) -> Result? {
