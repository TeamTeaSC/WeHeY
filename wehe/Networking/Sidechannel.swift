...
class Sidechannel {
    ...
    init?(address: String, port: Int, family: Socket.ProtocolFamily) {
        ...
        do {
            ...
            if Settings.https {
                let chainFilePath = String(Bundle.main.url(forResource: Settings.certFileName, withExtension: "pfx")!.absoluteString.dropFirst(7))
     ...
     func connect() throws {
        do {
            print("Client connects to: \(address), \(port)")
    ...
    func sortTimeSlices(slices: [Double: Double]) -> [[Double]] {
    ...
    func sendTimeSlices(slices: [Double: Double]) throws {
        let output = sortTimeSlices(slices: slices)
