...
class Replayer {
    ...
    let replayRunner: ReplayRunnerProtocol
    let replay: Replay
    ...
    // Used only by Localization to identify Replayer
...
//    let metaDatachannel: MetaDataChannel?
    ...
    // Localization: added optional parameter historyCount
    init(settings: Settings, deviceIP: String, replay: Replay, replayType: ReplayType, replayRunner: ReplayRunnerProtocol, app: App, serverIP: String?, serverID: ServerID? = nil, historyCount: Int? = nil) throws {
        self.settings = settings
        self.deviceIP = deviceIP
        self.replay = replay
        self.replayType = replayType
        self.replayRunner = replayRunner
        self.historyCount = historyCount ?? app.historyCount ?? 0
        self.app = app
...
//        }
    ...
    func runReplay(dpiTestID: Int = -1) {
        ...
        do {
            ...
            print("DEBUG: side channel declareID")
            try sidechannel.sendChangeSpec(mpacNum: -1, action: "null", spec: "null")
            print("DEBUG: side channel sendChangeSpec")
...
//                self.replayRunner.updateProgress(value: 0.7)
            ...
            Helper.runOnUIThread {
                self.replayRunner.updateProgress(value: 0, serverID: self.serverID)
    ...
    private func runUDPReplay(portMapping: PortMapping, clientIP: String, senderCount: Int, numberOfTimeSlices: Double, testTimeout: Double) throws {
        ...
        if spPair.ip == "" {
            // originally settings.serverIP
            spPair.ip = self.serverIP
        }
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            for (index, packet) in self.replay.packets.enumerated() {
                ...
                Helper.runOnUIThread {
                    self.replayRunner.updateProgress(value: progress, serverID: self.serverID)
                    }
            }
        ...
        do {
            ...
            print("DEBUG! ", averageThroughputs)
    ...
    private func runTCPReplay(portMapping: PortMapping, clientIP: String, numberOfTimeSlices: Double, testTimeout: Double) throws {
        ...
        // figure out the ip and port for the socket
        // TODO: investigate replay.destinationIP and replay.port
        ...
        do {
            ...
            try client.setWriteTimeout(value: socketWriteTimeout)
            try client.connect(to: spPair.ip, port: spPair.port, timeout: socketConnectTimeout)
            print("TCP connecting to \(spPair.ip):\(spPair.port)")
        } catch let error {
        ...
        Helper.runOnUIThread {
            self.replayRunner.updateProgress(value: 1.0, serverID: self.serverID)
        }
        ...
        do {
            ...
            print("DEBUG! ", averageThroughputs)
            if app.isPortTest {
                let sortedTimeSlices = sidechannel.sortTimeSlices(slices: averageThroughputs)
                let sortedAverageThroughputsSlice = sortedTimeSlices[0].dropLast(1)
                let sortedAverageThroughputs = Array(sortedAverageThroughputsSlice)
                // skip the last throughput, which might be skewed
                app.appThroughput = sortedAverageThroughputs.reduce(0, +) / Double(sortedAverageThroughputs.count)
            }
            try sidechannel.getResult()
        } catch let error {
            throw error
