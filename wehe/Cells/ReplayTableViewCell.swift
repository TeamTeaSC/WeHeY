...
class ReplayTableViewCell: UITableViewCell {
    ...
    private func updateText() {
        ...
        if let differentiation = app?.differentiation {
        ...
        if let localization = app?.localization {
            switch localization {
            case .inconclusive:
                differentiationTextField.text = LocConstants.Parse.inconclusive
            case .commonDiff:
                differentiationTextField.text = LocConstants.Parse.commonDiff
            case .noEvidence:
                differentiationTextField.text = LocConstants.Parse.noEvidence
            }
        }
    }
    ...
    private func updateColors() {
        ...
        } else if let localization = app?.localization {
            switch localization {
            case .inconclusive: differentiationTextField.textColor = UIColor.orange
            case .commonDiff: differentiationTextField.textColor = UIColor.red
            case .noEvidence: differentiationTextField.textColor = Settings.noDifferentiationColor
            }
        } else {
    ...
    private func updateVisibility() {
        ...
        if app?.differentiation != nil || app?.status == .willRerun || app?.status == .waitingForResults || app?.localization != nil {
        ...
        if app?.status == .receivedResults { // test finished, received results
        ...
        } else if app?.status == .receivedLoc || app?.status == .noBothDiff { // received localization results
            infoImageView.isHidden = true
            reportButton.isHidden = true
            arcepIconImageView.isHidden = true
            actualThroughputTextField.isHidden = true
            ...
            reportButton.isHidden = true
            arcepIconImageView.isHidden = true
        } else if app?.status == .locError { // localization error
            statusTextField.isHidden = true
            progressBar.isHidden = true
